...
def get_password_reset_email_template(
    ...
    """Get the HTML password reset email template with dynamic values.
    ...
    template = load_template("password_reset_email")

    # Add tenant name as query parameter to Panorama URL
    separator = "&" if "?" in panorama_url else "?"
    query_params = urlencode({"tenantname": tenant_name})
    panorama_url_with_tenant = f"{panorama_url}/reset-password{separator}{query_params}"
